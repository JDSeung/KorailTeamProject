package com.korail.client.fileupload;
...
public class FileUploadUtil {
	...
	public static String fileUpload(MultipartFile file, HttpServletRequest request) throws IOException{
		...
		if(org_name != null && (!org_name.equals(""))){
			...
			String docRoot = request.getSession().getServletContext().getRealPath("/resources/upload");
			System.out.println("docRoot name" + docRoot);
			File fileDir = new File(docRoot);
			if(!fileDir.exists()){
				fileDir.mkdirs(); // 폴더 생성
			}
